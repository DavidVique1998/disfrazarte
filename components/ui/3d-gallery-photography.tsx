...
function GalleryScene({ images, speed = 1, visibleCount = 8, disableScroll = false, fadeSettings = {
...
function FallbackGallery({ images }: { images: ImageItem[] }) {
  ...
  return (
    <div className="grid grid-cols-3 gap-2 p-4 h-full overflow-hidden">
      {normalized.map((img, i) => (
        <img key={i} src={img.src} alt={img.alt} className="w-full h-32 object-cover" />
      ))}
    </div>
  );
}
